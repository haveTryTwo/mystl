...
public:
    ...
    {
        return __my_deque_buf_size(bufsize, sizeof(T));
    }
    ...
    {
        difference_type distance = difference_type(buffer_size()) * (node - x.node - 1) +
...
protected:
    ...
    map_pointer map; // point to map, and this space is consistence, and the value of the map is pointer which 				 // pointer to the node
...
protected:
    ...
    void destroy_nodes();
public:
    size_type get_initial_map_size() const
    {
        return (size_type) initial_map_size;
    }
...
template <class T, class Alloc, size_t bufsize>
typename my_deque<T, Alloc, bufsize> ::iterator my_deque<T, Alloc, bufsize> ::erase(iterator first, iterator last)
{
...
template <class T, class Alloc, size_t bufsize>
typename my_deque<T, Alloc, bufsize> ::iterator my_deque<T, Alloc, bufsize> ::insert(iterator position,
        const value_type& x)
{
...
template <class T, class Alloc, size_t bufsize>
typename my_deque<T, Alloc, bufsize> ::iterator my_deque<T, Alloc, bufsize> ::insert_aux(iterator pos,
        const value_type& x)
{
    difference_type index = pos - start;
    value_type x_copy = x;
