...
public:
    ...
    explicit __my_rb_tree_iterator(link_type x)
    {
        node = x;
    }

    __my_rb_tree_iterator(const iterator& it)
    {
        node = it.node;
    }
...
protected:
    ...
    {
        rb_tree_node_allocator ::deallocate(x);
    }
...
// TO GET THE MAX AND MIX VALUE IN THE TREE
    ...
    {
        return (link_type) __my_rb_tree_node_base ::minimum(x);
    }

    static link_type maximum(link_type x)
    {
        return (link_type) __my_rb_tree_node_base ::maximum(x);
    }
...
public:
    my_rb_tree(const Compare& comp = Compare()) : node_count(0), key_compare(comp)
    {
        init();
    }
    ...
    my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc>&
    operator= (const my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc>& x)
    {
        if (this == &x)
        {
            return *this;
...
public:
    ...
    void insert_equal(iterator position, const value_type& x)
    {
        insert_equal(x);
    }
...
void my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::swap(my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc>& x)
{
    // swap the Compare
...
my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::lower_bound(link_type x, link_type y, const Key& k)
{
    while (x != 0)
    {
...
my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::lower_bound(link_type x, link_type y, const Key& k) const
{
    while (x != 0)
    {
        if (!key_compare(key(x), k))
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
typename my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator
my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::upper_bound(link_type x, link_type y, const Key& k)
{
    while (x != 0)
    {
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
typename my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::const_iterator
my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::upper_bound(link_type x, link_type y, const Key& k) const
{
    while (x != 0)
    {
        if (key_compare(k, key(x)))
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
typename my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::iterator
my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::__insert(base_ptr _x, base_ptr _y, const Value& v)
{
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
void my_rb_tree<Key, Value, KeyOfValue, Compare, Alloc> ::erase(const_iterator first, const_iterator last)
{
    if (first == begin() && last == end())
    {
        clear();
...
inline void __my_rb_tree_insert_rebalance(__my_rb_tree_node_base* x, __my_rb_tree_node_base*& root)
{
