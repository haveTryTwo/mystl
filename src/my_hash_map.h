#ifndef MY_HASH_MAPH_H
#define MY_HASH_MAPH_H

#include "my_hashtable.h"

template <class Key, class T, class HashFun = my_hash<Key>, class EqualKey = my_equal_to<Key>, class Alloc = my_alloc>
class my_hash_map
{
private:
...
public:
    ...
    template <class InputIterator>
    my_hash_map(InputIterator first, InputIterator last, size_type n) : rep(n, hasher(), key_equal())
    {
        rep.insert_unique(first, last);
    }

    template <class InputIterator>
    my_hash_map(InputIterator first, InputIterator last, size_type n, const hasher& hf) : rep(n, hf, key_equal())
    {
        rep.insert_unique(first, last);
    }

    template <class InputIterator>
    my_hash_map(InputIterator first, InputIterator last, size_type n, const hasher& hf, const key_equal& eql)
        : rep(n, hf, eql)
    {
        rep.insert_unique(first, last);
    }
...
public:
    ...
    friend bool operator== (const my_hash_map<K, DataType, H, E, A>& x, const my_hash_map<K, DataType, H, E, A>& y);
};
